...
public class DefaultRequestProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            ...
            /*如果请求码是 注册broker,路由注册*/
            case RequestCode.REGISTER_BROKER:
                Version brokerVersion = MQVersion.value2Version(request.getVersion());
                if (brokerVersion.ordinal() >= MQVersion.Version.V3_0_11.ordinal()) {
                    return this.registerBrokerWithFilterServer(ctx, request);
                } else {
                    // 参数1： 包装serverHandler的 ctx
    ...
    private boolean checksum(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        //  requestHeader.getBodyCrc32() 这个值 是客户端那边计算出来，保存到header中的
        if (requestHeader.getBodyCrc32() != 0) {
            // 服务端再次根据 body计算 crc
            final int crc32 = UtilAll.crc32(request.getBody());
            // 两次crc 不一致， 说明服务器拿到的request 和客户端发送的 request 数据不一致
    ...
    public RemotingCommand registerBroker(ChannelHandlerContext ctx,
        ...
        final RegisterBrokerResponseHeader responseHeader = (RegisterBrokerResponseHeader) response.readCustomHeader();
        // 反射创建 RegisterBrokerResponseHeader 对象，并且将 request.extFields 中的数据写入到 该对象中
        final RegisterBrokerRequestHeader requestHeader =
            (RegisterBrokerRequestHeader) request.decodeCommandCustomHeader(RegisterBrokerRequestHeader.class);
        ...
        if (!checksum(ctx, request, requestHeader)) {
            ...
            response.setCode(ResponseCode.SYSTEM_ERROR);
            response.setRemark("crc32 not match");
            // 返回response， 返回的response 由callback 对象处理
            return response;
        }

        // 执行到这里，说明 request 是正确的 包

        TopicConfigSerializeWrapper topicConfigWrapper;

        if (request.getBody() != null) {
            // 解码body，解码出来的数据就是 当前机器的 主题信息
        ...
        // 将结果信息 写道 responseHeader 中
        responseHeader.setHaServerAddr(result.getHaServerAddr());
        responseHeader.setMasterAddr(result.getMasterAddr());
        // 获取kv配置，写入 responseHeader中，kv配置是顺序消息相关的
        byte[] jsonValue = this.namesrvController.getKvConfigManager().getKVListByNamespace(NamesrvUtil.NAMESPACE_ORDER_TOPIC_CONFIG);
        response.setBody(jsonValue);
        // code 设置为success
        response.setCode(ResponseCode.SUCCESS);
    ...
    public RemotingCommand getRouteInfoByTopic(ChannelHandlerContext ctx,
        ...
        /*根据topic名称从路由表获取相关信息封装到 TopicRouteData*/
        TopicRouteData topicRouteData = this.namesrvController.getRouteInfoManager().pickupTopicRouteData(requestHeader.getTopic());

        /*条件成立：说明找到了topic主题对应的路由信息*/
        if (topicRouteData != null) {
            /*条件成立：说明该主题 是顺序消息*/
            if (this.namesrvController.getNamesrvConfig().isOrderMessageEnable()) {
                /*则从namesrv kvConfig中查找关于顺序消息相关的配置，保存到 TopicRouteData中*/
        ...
        /*走到这里，说明根据topic主题没有找到对应的路由信息，设置响应码返回*/

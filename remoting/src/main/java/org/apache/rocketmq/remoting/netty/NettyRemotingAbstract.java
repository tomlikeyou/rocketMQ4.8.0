...
public abstract class NettyRemotingAbstract {
    ...
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        ...
        // 如果未找到 code对应的 pair，则使用缺省的处理器，namesrv 并未注册 code 对应的处理器，使用缺省的
        ...
        if (pair != null) {
            ...
            try {
                // 将runnable  和 channel 和请求cmd一起封装为 requestTask 对象
                final RequestTask requestTask = new RequestTask(run, ctx.channel(), cmd);
                // 注意：这里从 IO线程 切换到 业务线程了!
                // 获取处理器对应的线程池，将task 提交，task执行时，会执行 runnable.run()
    ...
    public void processResponseCommand(ChannelHandlerContext ctx, RemotingCommand cmd) {
        ...
        if (responseFuture != null) {
            ...
            if (responseFuture.getInvokeCallback() != null) {
                ...
                // 设置 future 结果，这一步会调用 countDownLatch.countdown()，将同步调用的业务线程 唤醒
    ...
    public void invokeAsyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis,
        ...
        if (acquired) {
            // 执行到这里，当前线程可以发起请求，服务器 请求客户端的 并发 并未达到 上限

            // once对象封装了 释放信号量的操作.
            final SemaphoreReleaseOnlyOnce once = new SemaphoreReleaseOnlyOnce(this.semaphoreAsync);
            //  计算 执行到这一步为止，耗费的时间
            long costTime = System.currentTimeMillis() - beginStartTime;
            // 条件成立：说明已经超时了... 就不用再发起 rpc了...
            ...
            // 将 ResponseFuture 加入 响应 映射表中，key 其实是 RequestID,
            this.responseTable.put(opaque, responseFuture);

            try {
                // 1.将数据交给netty，netty IO线程 接管 写和刷 数据的操作
                // 2. 注册 写 和 刷 操作监听器，由 IO线程回调.
                channel.writeAndFlush(request).addListener(new ChannelFutureListener() {
                    @Override
                    public void operationComplete(ChannelFuture f) throws Exception {
                        if (f.isSuccess()) {
                            //写刷成功，设置  responseFuture 发生状态为true
                            responseFuture.setSendRequestOK(true);
                            return;
                        }
                        // 执行到这里，就是发送失败
            ...
            // 执行到这里 说明 获取信号量失败，说明当前服务器 并发比较高.
            if (timeoutMillis <= 0) {
